...
import { Declaration, declarationsdb } from '@/utils/common.utils';
...
export default function ClaimDeclarationCard() {
  ...
  const {
    ...
    isError,
    error,
  } = useQuery<Declaration[], Error>({
    queryKey: ['getDeclaration'],
    queryFn: () => declarationsdb,
    staleTime: 1000 * 60 * 5,
  });
  ...
  return (
    <>
      ...
      {!isLoading &&
        declarations &&
        declarations.map((declaration, decIndex) => (
          <Accordion
            key={decIndex}
            className="mt-4 border-none bg-white hover:bg-white"
          >
            <AccordionPanel>
              <AccordionTitle className="cursor-pointer bg-white hover:bg-white focus:ring-transparent">
                <div className="flex items-center gap-3">
                  <div className="flex h-8 min-w-8 items-center justify-center rounded-full border-3 border-blue-500 text-sm font-bold text-black">
                    {decIndex + 1}{' '}
                    <span className="text-xs">/{declarations.length}</span>
                  </div>
                ...
                <div className="overflow-x-auto">
                  <Table hoverable>
                    ...
                    <TableBody className="divide-y">
                      {declaration.deposits.map((deposit, depIndex) => (
                        <TableRow
                          key={depIndex}
                          className="border-b-1 border-b-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800"
                        >
                ...
                <div className="mt-4 rounded-b-lg bg-gray-50 p-5">
                  ...
                  {!isOpen && (
                    <div className="mt-2 text-xs text-gray-500 italic">
                      To upload or view attachments, you may expand the section
                      by clicking above icon
                    </div>
                  )}
                  ...
                  {isOpen && (
                    <div className="mt-4 text-gray-500">
                      <div className="text-sm lg:w-4/6">
                        ...
                        submission. Make sure your files are clear, complete,
